...
/* Current 22-bit payload to embed in the RPA; updated via buttons */
static uint32_t current_payload = 0;
...
/* ---------- Re-derive RPA from current_payload and push to controller ---------- */
/* Caller must ensure scanning is stopped before calling this. */
static void update_rpa_from_payload(void)
{
    bt_addr_t rpa;
    int err;

    build_rpa(current_payload, &rpa);
    LOG_INF("New payload=%u -> MAC: %02X:%02X:%02X:%02X:%02X:%02X",
            current_payload,
            rpa.val[5], rpa.val[4], rpa.val[3], rpa.val[2], rpa.val[1], rpa.val[0]);

    err = hci_set_random_address(&rpa);
    if (err) {
        LOG_ERR("Failed to update random address (err %d)", err);
    }
}

/* ---------- Button handler: stop scan, update address, restart scan ---------- */
static void button_handler(uint32_t button_state, uint32_t has_changed)
{
    ...
    if (pressed & DK_BTN1_MSK) {
        ...
        current_payload = 1;
    } else if (pressed & DK_BTN3_MSK) {
        current_payload = 2;
    } else if (pressed & DK_BTN4_MSK) {
        current_payload = 3;
    } else {
...
int main(void)
{
    ...
    /* Set initial address for current_payload = 0 before scanning starts */
    update_rpa_from_payload();

    scan_setup(true);

    LOG_INF("Nordic Filtered Active Scanning live, payload=%u...", current_payload);

...
class TestSettings(ABC):
    ...
    def __init__(self,
        ...
        self.tensor_flow_reference_version = (
            "// Generated by {} using tensorflow version {}).\n".format(
                os.path.basename(__file__), tf.__version__))
    ...
    def set_output_dims_and_padding(self, output_x, output_y):
        ...
        if self.has_padding:
            # need to do this manually since the padding is inserted as
            # a separate operation so the input shape is the padded one
            self.pad_x = (self.x_input - self.x_output)//2
            self.pad_y = 0
